...
const ViewRequests = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://abhorrent-soda-production.up.railway.app/getAppointments')
      .then(response => {
  ...
  const handleDeleteAppointment = async (makeappointmentid) => {
    try {
      const response = await axios.delete(`https://abhorrent-soda-production.up.railway.app/deleteUserAppointment/${makeappointmentid}`);
